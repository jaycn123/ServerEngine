...
int64_t CppMySQLQuery::getInt64Field(int nField, int64_t nNullValue /*= 0*/)
{
	if ( NULL == m_MysqlRes )
	{
		return nNullValue;
	...
	return std::stol(_row[nField]);
}

int64_t CppMySQLQuery::getInt64Field(const char* szField, int64_t nNullValue /*= 0*/)
{
	if ( NULL == m_MysqlRes || NULL == szField )
	{
		return nNullValue;
...
int64_t CppMySQL3DB::getAutoIncrementID(const char* szTableName, const char* szDBName)
{
	...
	int64_t tId = tQuery.getInt64Field(0);
	return tId;
}
