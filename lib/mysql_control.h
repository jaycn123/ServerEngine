...
private:  
	...
	std::map<std::string,int>m_FieldIndexMap;
...
public:  
	...
	{  
		...
		m_FieldIndexMap[m_Field->name] = 0;
		if( NULL == m_Result )  
			return -1;  
		m_RowCount = mysql_num_rows(m_Result);  
		m_ColumnCount = mysql_num_fields(m_Result);  
		
		for(auto i = 1;i < m_ColumnCount; i++)
		{
			m_Field = mysql_fetch_field(m_Result);
		    m_FieldIndexMap[m_Field->name] = i;
		}
		
		mysql_field_seek(m_Result, 0);//定位到第0列
		m_Field = mysql_fetch_field(m_Result);
